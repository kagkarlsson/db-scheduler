...
Registers metrics for all library (task, candidates due, housekeeping) executors. Currently supported only for Micrometer in Spring Boot (see [metric names](https://docs.micrometer.io/micrometer/reference/reference/jvm.html)). Default `false`.
**NB:** Requires `--add-opens java.base/java.util.concurrent=ALL-UNNAMED` JVM setting for custom user executors (required by [`MicrometerStatsRegistry`](https://github.com/micrometer-metrics/micrometer/blob/main/micrometer-core/src/main/java/io/micrometer/core/instrument/binder/jvm/ExecutorServiceMetrics.java)).
