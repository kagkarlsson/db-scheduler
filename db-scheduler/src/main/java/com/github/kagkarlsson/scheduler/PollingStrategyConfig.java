...
public class PollingStrategyConfig {
    ...
    public static final PollingStrategyConfig DEFAULT_SELECT_FOR_UPDATE = new PollingStrategyConfig(Type.LOCK_AND_FETCH,
            0.5, 1.0);
    ...
    public PollingStrategyConfig(Type type, double lowerLimitFractionOfThreads, double upperLimitFractionOfThreads) {
        ...
        if (lowerLimitFractionOfThreads >= upperLimitFractionOfThreads) {
            throw new IllegalArgumentException(
                    "lowerLimitFractionOfThreads should be lower than upperLimitFractionOfThreads");
        }

        if (upperLimitFractionOfThreads < 1.0) {
            throw new IllegalArgumentException(
                    "upperLimit should be equals to number of threads or higher, i.e. fraction higher than 1");
        }
    }

    public String describe() {
        return "type=" + type.name() + ", lowerLimit=" + lowerLimitFractionOfThreads + ", upperLimit="
