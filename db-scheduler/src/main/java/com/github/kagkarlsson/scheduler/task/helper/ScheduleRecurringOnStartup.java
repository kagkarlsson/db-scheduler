...
class ScheduleRecurringOnStartup<T> implements ScheduleOnStartup<T> {
    ...
    public void apply(SchedulerClient scheduler, Clock clock, Task<T> task) {
        ...
        final Optional<ScheduledExecution<Object>> preexistingExecution = scheduler
                .getScheduledExecution(instanceWithoutData);
        ...
        if (preexistingExecution.isPresent()) {
            Optional<Instant> newNextExecutionTime = checkForNewExecutionTime(clock, instanceWithoutData,
                    preexistingExecution.get());

            newNextExecutionTime.ifPresent(instant -> {
                // Intentionally leaving out data here, since we do not want to update existing
                // data
            ...
            LOG.info("Creating initial execution for task-instance '{}'. Next execution-time: {}", instanceWithoutData,
    ...
    Optional<Instant> checkForNewExecutionTime(Clock clock, TaskInstance<T> instanceWithoutData,
        ...
        final Instant nextExecutionTimeRelativeToNow = schedule
                .getNextExecutionTime(ExecutionComplete.simulatedSuccess(clock.now()));

        if (preexistingExecutionTime.isBefore(clock.now().plus(Duration.ofSeconds(10)))) {
            // Only touch future executions as a precaution to avoid warnings/noise related
            // to execution being run by the scheduler
            LOG.debug("Not checking if task-instance '{}' needs rescheduling as its execution-time is too near now().",
                    instanceWithoutData);
            return Optional.empty();

        } else if (schedule.isDeterministic()) {
            final Instant potentiallyNewExecutionTime = schedule
                    .getNextExecutionTime(ExecutionComplete.simulatedSuccess(clock.now()));
            // Schedules: Cron, Daily
            if (differenceGreaterThan(preexistingExecutionTime, potentiallyNewExecutionTime, Duration.ofSeconds(1))) {
                // Deterministic schedule must have changed (note: may be nuances here depending
                // on precision of database timestamp)
                LOG.info(
            ...
            // The schedule has likely been updated, rescheduling to next execution-time
            // according to schedule
            // We cannot reschedule if new execution-time is further into the future than
            // existing since that may cause
            // us to never run executions if scheduler restarts is more frequent than the
    ...
    static boolean differenceGreaterThan(Instant preexistingExecutionTime, Instant potentiallyNewExecutionTime,
            Duration delta) {
