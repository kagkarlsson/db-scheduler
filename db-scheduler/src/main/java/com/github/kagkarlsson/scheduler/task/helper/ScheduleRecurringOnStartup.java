...
class ScheduleRecurringOnStartup<T> implements ScheduleOnStartup<T> {
    ...
    Optional<Instant> checkForNewExecutionTime(
            Clock clock, TaskInstance<T> instanceWithoutData, ScheduledExecution<Object> preexistingExecution) {
    ...
    static boolean differenceGreaterThan(
        ...
        final Duration difference = Duration.between(preexistingExecutionTime, potentiallyNewExecutionTime)
                .abs();
