...
public class UnrecognizableScheduleTest {
  ...
  public void contains_correct_message() {
    ...
    assertMessage(
        null,
        Arrays.asList("PARSABLE 1", "PARSABLE 2"),
  ...
  private static void assertMessage(String scheduleString, String expectedMessage) {
    assertEquals(
        expectedMessage, new Schedules.UnrecognizableSchedule(scheduleString).getMessage());
  }

  private static void assertMessage(
      String scheduleString, List<String> examples, String expectedMessage) {
    assertEquals(
        expectedMessage,
        new Schedules.UnrecognizableSchedule(scheduleString, examples).getMessage());
  }
}
