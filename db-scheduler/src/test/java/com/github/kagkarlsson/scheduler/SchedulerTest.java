...
public class SchedulerTest {
    ...
    // new
    // ChangeLogLevelsExtension.LogLevelOverride("com.github.kagkarlsson.scheduler",
    // Level.TRACE)
    // );
    ...
    private ManualScheduler schedulerFor(Task<?>... tasks) {
        ManualScheduler manualScheduler = TestHelper.createManualScheduler(postgres.getDataSource(), tasks).clock(clock)
                .start();
    ...
    public void scheduler_should_track_duration() throws InterruptedException {
        ...
        Scheduler scheduler = Scheduler.create(postgres.getDataSource(), oneTimeTask).threads(2)
                .pollingInterval(Duration.ofMillis(100)).schedulerName(new SchedulerName.Fixed("test")).build();
    ...
    public void should_expose_cause_of_failure_to_completion_handler() throws InterruptedException {
        ...
        Task<Void> oneTimeTask = ComposableTask.customTask("cause-testing-task", Void.class,
                TestTasks.REMOVE_ON_COMPLETE, failureHandler, (inst, ctx) -> {
    ...
    public void should_only_attempt_task_when_max_retries_handler_used() throws InterruptedException {
        ...
        FailureHandler.MaxRetriesFailureHandler<Void> failureHandler = new FailureHandler.MaxRetriesFailureHandler<Void>(
                maxRetries, new FailureHandler.OnFailureRetryLater<>(ofMinutes(1)));

        OneTimeTask<Void> oneTimeTask = Tasks.oneTime("max-retries-task").onFailure(failureHandler)
                .execute((inst, ctx) -> {
        ...
        // Simulate 15 minutes worth of time to validate we did not process more than we
        // should
    ...
    public void should_reschedule_failure_on_exponential_backoff_with_default_rate() throws InterruptedException {
        ...
        // Simulate 30 minutes worth of time to validate we did not process more than we
        // should
        ...
        // Skip first execution of this b/c it was not using the exponential backoff but
        // the first attempted call before failure
        for (int i = 1, executionTimesSize = executionTimes.size(); i < executionTimesSize; i++) {
            ...
            assertThat(scheduleTimeDifferenceFromFirstCall.getSeconds(),
