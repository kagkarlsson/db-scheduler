...
class QueryBuilderTest {
    ...
    void test() {
        ...
        assertNonWhitespaceEquals("select * from table1 order by c1 asc",
                selectFromTable("table1").orderBy("c1 asc").getQuery());

        assertNonWhitespaceEquals("select * from table1 where field1=?",
                selectFromTable("table1").andCondition(stringField("field1", "a")).getQuery());

        assertNonWhitespaceEquals("select * from table1 where field1=? and field2=?", selectFromTable("table1")
                .andCondition(stringField("field1", "a")).andCondition(stringField("field2", "b")).getQuery());
