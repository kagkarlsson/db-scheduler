...
public class NoAutocommitTest {
    ...
    public void clearTables() {
        assertTimeoutPreemptively(Duration.ofSeconds(20), () -> DbUtils.clearTables(noAutocommitingDatasource));
    }

    @Test
    public void onstartup_should_always_commit() {
        final TestTasks.CountingHandler<Void> counter = new TestTasks.CountingHandler<>();
        final RecurringTask<Void> recurring1 = Tasks.recurring("recurring1", new FutureSchedule()).execute(counter);

        // Setup non auto-committing datasource
        Scheduler scheduler = Scheduler.create(noAutocommitingDatasource).startTasks(recurring1)
                .schedulerName(new SchedulerName.Fixed("test")).commitWhenAutocommitDisabled(false).build();
        stopScheduler.register(scheduler);

        scheduler.start();

        final Optional<ScheduledExecution<Object>> scheduledExecution = scheduler
                .getScheduledExecution(recurring1.getDefaultTaskInstance());
