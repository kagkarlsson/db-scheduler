...
public class SchedulesTest {
  private static final Instant NOON_TODAY =
      ZonedDateTime.now().withHour(12).withMinute(0).withSecond(0).withNano(0).toInstant();
  private static final Instant NOON_TODAY_ROME =
  ...
  public void should_validate_pattern() {
    ...
    assertThat(
        dailySchedule.getNextExecutionTime(complete(NOON_TODAY)),
        is(NOON_TODAY.plus(Duration.ofHours(1))));

    assertParsable("DAILY|12:00|Europe/Rome", Daily.class);
    Schedule dailyScheduleWithTimezone =
        assertParsable("DAILY|10:00,14:00|Europe/Rome", Daily.class);
    assertThat(
    ...
    assertThat(
        fixedDelaySchedule.getNextExecutionTime(complete(NOON_TODAY)),
        is(NOON_TODAY.plusSeconds(10)));
