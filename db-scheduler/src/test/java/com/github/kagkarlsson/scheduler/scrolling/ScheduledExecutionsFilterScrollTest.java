...
public class ScheduledExecutionsFilterScrollTest {
  ...
  void testScrollingAndLimiting() {
    ...
    var filter = active().after(afterBoundary).before(beforeBoundary).limit(100);
  ...
  @Test
  void testValidation() {
    assertThrows(NullPointerException.class, () -> active().after(null));
    assertThrows(NullPointerException.class, () -> active().before(null));

    assertThrows(IllegalArgumentException.class, () -> active().limit(0));
    assertThrows(IllegalArgumentException.class, () -> active().limit(-1));
  }
}
