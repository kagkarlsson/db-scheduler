...
class ScheduleRecurringOnStartupTest {
  ...
  private static final Instant UPCOMING_INSTANT_A =
      ZonedDateTime.of(DATE, TIME_INSTANT_A, ZONE).toInstant();
  private static final Instant NOW = UPCOMING_INSTANT_A.minus(Duration.ofHours(1));

  private static final String INSTANCE = "instance";
  private static final TaskInstance<Void> TASK_INSTANCE =
      new TaskInstance<>("recurring-a", INSTANCE);
  ...
  void checkForNewExecutionTime_no_new_execution_time_if_execution_imminent() {
    assertNotUpdatedExecutionTime(
        scheduled(NOW.plus(Duration.ofSeconds(1))), daily(TIME_INSTANT_A.plusMinutes(1)));
    assertUpdatedExecutionTime(
        scheduled(NOW.plus(Duration.ofSeconds(11))), daily(TIME_INSTANT_A.plusMinutes(1)));
  }
  ...
  void checkForNewExecutionTime_updated_if_deterministic_schedule_change() {
    ...
    assertNotUpdatedExecutionTime(
        scheduled(UPCOMING_INSTANT_A),
        daily(TIME_INSTANT_A.plus(Duration.ofMillis(20)))); // requires 1s diff between instants
    assertUpdatedExecutionTime(
        scheduled(UPCOMING_INSTANT_A), daily(TIME_INSTANT_A.plus(Duration.ofSeconds(20))));
    assertUpdatedExecutionTime(
        scheduled(UPCOMING_INSTANT_A), daily(TIME_INSTANT_A.minus(Duration.ofSeconds(20))));
  }
  ...
  private void assertUpdatedExecutionTime(
      ScheduledExecution<Object> scheduled, Schedule newSchedule) {
    final ScheduleRecurringOnStartup<Void> unit =
        new ScheduleRecurringOnStartup<>(INSTANCE, null, newSchedule);
    assertThat(unit.checkForNewExecutionTime(clock, TASK_INSTANCE, scheduled), not(empty()));
  }

  private void assertNotUpdatedExecutionTime(
      ScheduledExecution<Object> scheduled, Schedule newSchedule) {
    final ScheduleRecurringOnStartup<Void> unit =
        new ScheduleRecurringOnStartup<>(INSTANCE, null, newSchedule);
