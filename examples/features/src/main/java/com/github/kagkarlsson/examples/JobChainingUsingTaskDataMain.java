    ...
    public void run(DataSource dataSource) {
        ...
        final CustomTask<JobState> chainingTask = Tasks.custom("job-chain-poc", JobState.class)
                .execute((taskInstance, executionContext) -> {
                    ...
                    // For illustration, can be made less verbose using suitable
                    // abstractions etc
                    if (taskInstance.getData().currentStep == Step.STEP1) {
                        ...
                        return (executionComplete, executionOperations) -> {
                            ...
                            // executionContext.triggerCheckForDueExecutions() to
                            // hint at immediate execution
                        };
        ...
        final Scheduler scheduler = Scheduler.create(dataSource, chainingTask)
                .enableImmediateExecution() // Bug: currently no effect, only works when
                // using
                // schedulerClient
        ...
        scheduler.schedule(
                chainingTask.instance("job-507", JobState.newJob(507)),
