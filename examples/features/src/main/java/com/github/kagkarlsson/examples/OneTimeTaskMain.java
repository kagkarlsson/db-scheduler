...
public class OneTimeTaskMain extends Example {
    ...
    public void run(DataSource dataSource) {
        ...
        final Scheduler scheduler =
                Scheduler.create(dataSource, myAdhocTask).threads(5).build();

        scheduler.start();

        // Schedule the task for execution a certain time in the future and optionally provide
        // custom
        // data for the
        // execution
        scheduler.schedule(
                myAdhocTask.instance("1045", new MyTaskData(1001L)),
