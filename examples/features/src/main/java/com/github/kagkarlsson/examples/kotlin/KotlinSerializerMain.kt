...
fun main() {
    ...
    val myAdhocTask: OneTimeTask<JsonData> = Tasks.oneTime("json-task", JsonData::class.java)
            .execute(VoidExecutionHandler { inst: TaskInstance<JsonData>, _: ExecutionContext? -> println("Executed! Custom data: " + inst.data) })
    val scheduler = Scheduler
            ...
            .build()
    scheduler.start()
    scheduler.schedule(myAdhocTask.instance("id1", JsonData(1001L, Instant.now().toEpochMilli())), Instant.now().plusSeconds(5))
}
