...
public class MaxRetriesMain extends Example {
    ...
    public void run(DataSource dataSource) {
        ...
        OneTimeTask<Void> failingTask = Tasks.oneTime("max_retries_task").onFailure(
                new FailureHandler.MaxRetriesFailureHandler<>(3, (executionComplete, executionOperations) -> {
        ...
        final Scheduler scheduler = Scheduler.create(dataSource, failingTask).pollingInterval(Duration.ofSeconds(2))
                .build();
