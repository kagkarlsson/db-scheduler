...
public class MaxRetriesMain extends Example {
    ...
    public void run(DataSource dataSource) {
        ...
        OneTimeTask<Void> failingTask = Tasks.oneTime("max_retries_task")
                .onFailure(new MaxRetriesFailureHandler<>(3, (executionComplete, operations) -> {
                    // try again in 1 second
                    System.out.println("Execution has failed "
                            + executionComplete.getExecution().consecutiveFailures
                            + " times. Trying again in a bit...");
                    operations.reschedule(executionComplete, Instant.now().plusSeconds(1));
                }))
                .execute((taskInstance, executionContext) -> {
                    throw new RuntimeException("simulated task exception");
                });

        final Scheduler scheduler = Scheduler.create(dataSource, failingTask)
