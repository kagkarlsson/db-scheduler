...
public class RecurringStateTrackingConfiguration {
    ...
    public static final TaskWithDataDescriptor<Integer> STATE_TRACKING_RECURRING_TASK = new TaskWithDataDescriptor<>(
            "state-tracking-recurring-task", Integer.class);

    /** Start the example */
    public static void start(ExampleContext ctx) {
        Integer data = 1;
        ctx.log("Starting recurring task " + STATE_TRACKING_RECURRING_TASK.getTaskName() + " with initial data: " + data
                + ". Initial execution-time will be now (deviating from defined schedule).");

        ctx.schedulerClient.schedule(STATE_TRACKING_RECURRING_TASK.instance(RecurringTask.INSTANCE, data), Instant.now() // start-time,
                                                                                                                         // will
                                                                                                                         // run
                                                                                                                         // according
                                                                                                                         // to
    ...
    public Task<Integer> stateTrackingRecurring() {
        return Tasks.recurring(STATE_TRACKING_RECURRING_TASK, Schedules.cron("0/5 * * * * *")).doNotScheduleOnStartup() // just
                                                                                                                        // for
                                                                                                                        // demo-purposes,
                                                                                                                        // so
                                                                                                                        // we
                ...
                .executeStateful((TaskInstance<Integer> taskInstance, ExecutionContext executionContext) -> {
                    EventLogger.logTask(STATE_TRACKING_RECURRING_TASK,
