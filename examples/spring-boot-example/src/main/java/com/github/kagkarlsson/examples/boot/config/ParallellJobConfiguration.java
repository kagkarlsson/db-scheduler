...
public class ParallellJobConfiguration {
    ...
    public static final TaskWithoutDataDescriptor PARALLEL_JOB_SPAWNER = new TaskWithoutDataDescriptor(
            "parallel-job-spawner");
    public static final TaskWithDataDescriptor<Integer> PARALLEL_JOB = new TaskWithDataDescriptor<>("parallel-job",
            Integer.class);
    ...
    public static void start(ExampleContext ctx) {
        ctx.log("Starting recurring task " + PARALLEL_JOB_SPAWNER.getTaskName()
                + ". Initial execution-time will be now (deviating from defined schedule).");

        ctx.schedulerClient.reschedule(PARALLEL_JOB_SPAWNER.instanceId(RecurringTask.INSTANCE), Instant.now());
    }

    /** Bean definition */
    @Bean
    public Task<Void> parallelJobSpawner() {
        return Tasks.recurring(PARALLEL_JOB_SPAWNER, Schedules.cron("0/20 * * * * *")).doNotScheduleOnStartup() // just
                                                                                                                // for
                                                                                                                // demo-purposes,
                                                                                                                // so we
                                                                                                                // can
                ...
                .execute((TaskInstance<Void> taskInstance, ExecutionContext executionContext) -> {
                    ...
                    // Create all or none. SchedulerClient is transactions-aware since a Spring
                    // datasource is used
    ...
    public Task<Integer> parallelJob() {
        return Tasks.oneTime(PARALLEL_JOB)
                .execute((TaskInstance<Integer> taskInstance, ExecutionContext executionContext) -> {
                    ...
                    EventLogger.logTask(PARALLEL_JOB,
                            String.format("Ran. Generated report for quarter Q%s  (in thread '%s', duration %sms)",
