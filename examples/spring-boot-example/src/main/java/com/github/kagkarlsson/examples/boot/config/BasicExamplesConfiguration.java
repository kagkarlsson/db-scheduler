...
public class BasicExamplesConfiguration {
    ...
    public static final TaskWithoutDataDescriptor BASIC_ONE_TIME_TASK = new TaskWithoutDataDescriptor(
            "sample-one-time-task");
    public static final TaskWithoutDataDescriptor BASIC_RECURRING_TASK = new TaskWithoutDataDescriptor(
            "recurring-sample-task");
    ...
    /**
     * Define a recurring task with a dependency, which will automatically be picked
     * up by the Spring Boot autoconfiguration.
     */
    @Bean
    Task<Void> recurringSampleTask(CounterService counter) {
        return Tasks.recurring(BASIC_RECURRING_TASK, fixedDelay(Duration.ofMinutes(1))).execute((instance, ctx) -> {
